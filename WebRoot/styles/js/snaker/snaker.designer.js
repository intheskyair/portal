(function(snaker){
	...
	designer.rect=function(o, r){
        ...
        _name = _r.text(
            ...
            .attr(_o.name);
        if(_o.text.text == designer.config.displayName){
        	_o.text.fill = "#ff0000";
        }
        _text = _r.text(
                _o.attr.x + _o.img.width + (_o.attr.width - _o.img.width) / 2,
	...
	designer.props=function(o, r){
		var _this = this, _pdiv = snaker("#properties").hide().draggable({handle:"#properties_handle"}).resizable()
		.css(designer.config.props.attr).bind("click",function(){return false}),
	...
	designer.init=function(c, o){
		...
		if(designer.config.editable){
			...
			snaker("#back").click(function(){
				designer.config.tools.back.onclick()
			});
