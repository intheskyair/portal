...
public class ProposalDao extends DbOperator{
	
	private static ProposalDao instance = null;
	...
	public List<Map<String, Object>> getExportDara(Map<String, Object> map) throws UnsupportedEncodingException 
	 {
			...
			String sql = "select a.* from kh_proposal a  "
	 ...
	 * @return
	 */
	public OraPaginatedList queryProject_jys(Map<String, Object> map,int pageNum,int pageSize) 
	{
		String condition=this.getDesSql(map);
		String conditionP = "";
		String param = "";
	 ...
	 * @return
	 */
	public boolean saveProject_jys(Map<String, Object> map,String uid)
	{
		StringBuilder sb=new StringBuilder();
		sb.append("insert into kh_proposal(title,code,unit,accept,approval,uid,filename,filepath,createtime) values(?,?,?,?,?,?,?,?,?) ");
	 ...
	 * @return
	 */
	public boolean updateProject_jys(Map<String, Object> map,String uid)
	{

		StringBuilder sb=new StringBuilder();
		sb.append("update kh_proposal set title=?,code=?,unit=?,accept=?,approval=?,uid=?,filename=?,filepath=? where id =? ");
		Object[] object = new Object[9];
		object[0] = StringHelper.noEmpty2(map.get("title")).trim();
		object[1] = StringHelper.noEmpty2(map.get("code")).trim();
		object[2] = StringHelper.noEmpty2(map.get("unit")).trim();
	 ...
	 * @return
	 */
	public boolean deleteProject_jys(String id)
	{	
		StringBuilder sb=new StringBuilder();
		sb.append("delete from kh_proposal where id =? ");
