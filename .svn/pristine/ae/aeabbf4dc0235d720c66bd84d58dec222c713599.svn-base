...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于显示的Panel模板
        //调整成自适应的iframe,并加上最小宽度
        tabPanelTemplate: '<div id="cleverTabPanelItem-#{id}" style="height: 100%;overflow:hidden;"><iframe frameBorder="0" style="width: 100%; display: inline; height: 100%;overflow:hidden;" src="#{src}"></iframe></div>',
        //Tab唯一id生成器
...
CleverTabs.prototype.add = function (options) {
    ...
    tabHeader.bind("click", function () {
        if (!$(this).hasClass('ui-state-disabled')) {
            tab.activate();
//###########tab激活的同时，再刷新一次，为避免火狐中初始化含有datagrid数据表格的页面没加载    20160615
            tab.refresh();
        }
    });
