...
<html>
	...
	<body>
	<form id="mainForm" action="${ctx}/process!listS.action" method="post">
				...
				<tr>
					...
					<td class="td_list_2" align=left nowrap>
					   <c:if test="${process.state == 1 }">
						 <a href="${ctx}${process.instanceUrl }?processId=${process.id }&processName=${process.name }" class="btnStart" title="启动流程">发起流程</a> 
						</c:if>
					</td>
				</tr>
			</c:forEach>
