...
public class FeasibilityDao extends DbOperator{
	
	private static FeasibilityDao instance = null;
	public static FeasibilityDao getInstance()
	{
		if(instance==null)
		{
			synchronized (ClientDao.class)
			{
			  instance = new FeasibilityDao();
			}
		}
		return instance;
	}
	...
	public List<Map<String, Object>> getExportDara(Map<String, Object> map) throws UnsupportedEncodingException 
	 {
			...
			String sql = "select a.* from kh_feasibility a  "
	 ...
	 * @return
	 */
	public OraPaginatedList queryProject_kxx(Map<String, Object> map,int pageNum,int pageSize) 
	{
		String condition=this.getDesSql(map);
		String conditionP = "";
		String param = "";
	 ...
	 * @return
	 */
	public boolean addProject_kxx(Map<String, Object> map,String uid)
	{
		StringBuilder sb=new StringBuilder();
		sb.append("insert into kh_feasibility(title,code,unit,accept,approval,uid,filename,filepath,createtime) values(?,?,?,?,?,?,?,?,?) ");
	 ...
	 * @return
	 */
	public boolean updateProject_kxx(Map<String, Object> map,String uid)
	{

		StringBuilder sb=new StringBuilder();
		sb.append("update kh_feasibility set title=?,code=?,unit=?,accept=?,approval=?,uid=?,filename=?,filepath=? where id =? ");
		Object[] object = new Object[9];
		object[0] = StringHelper.noEmpty2(map.get("title")).trim();
		object[1] = StringHelper.noEmpty2(map.get("code")).trim();
		object[2] = StringHelper.noEmpty2(map.get("unit")).trim();
	 ...
	 * @return
	 */
	public boolean deleteProject_kxx(String id)
	{	
		StringBuilder sb=new StringBuilder();
		sb.append("delete from kh_feasibility where id =?");
