		 ...
		 var param_lczl = {
			...
			bindParamGrid:function(queryP,queryM){
				param_grid_user = $("#dcdb_grid").flexigrid({
					url:"lczl!LczlList_pro.action?operation=init&processId=${processId}",
					colModel: queryM,
					buttons : queryP,
					rowId:"id",
					showToggleBtn: true,
			...
			if($('.trSelected', grid).length==1)
						{
							...
							var path = $('.trSelected', grid).eq(0).find("td:eq(9)").text();
							for(var i = 0; i < 2 ; i++)
							{
								if(i == 0){
									com = '考核表单';
								...
								if(com == '考核表单'){
									url = "borrow!view.action";
									 data = {"orderId" : orderId,"processId":pid,"path":path};
								}else{
