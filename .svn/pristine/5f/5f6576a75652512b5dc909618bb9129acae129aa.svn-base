				...
				$("#flowTable_rece").html("");
							...
							$.ajax({
								...
								{	
								    ...
								    $("#flowTable_rece").append(result_str);
								    }else{
								    $("#formTable_rece").append(result);
								    }
								    $("#view_rece .tab li").click(function()
								    {
										$("#view_rece .tab li").removeClass("current");
										$(this).addClass("current");
										var n = $(this).index();
										$("#view_rece .content_y ul").hide();
										$("#view_rece .content_y ul").eq(n).show();
		                            });
								}								
							});
...
<body style="height:100%;overflow-y: scroll;">
	<form id="inputForm1" action="" method="post" target="mainFrame">
		<input type="hidden" name="processId" value="${map.processId }" /> <input
			type="hidden" name="orderId" value="${map.orderId }" /> <input
			type="hidden" name="taskId" value="${map.taskId }" /> <input
			type="hidden" name="taskName" value="${map.taskName }" /> <input
			type="hidden" name="userList" id="userList" />
		<fieldset>
			<legend>【${map.label}】</legend>
			<table class="table_all" align="center" border="0" cellpadding="0"
				cellspacing="0" style="margin-top: 0px">
				<tr>
					<td class="td_table_1"><span>领导审批结果：</span></td>
					<td class="td_table_2" colspan="3"><input type="radio"
						id="agree" name="method" value="0" checked="checked" />同意 <input
						type="radio" id="noagree" name="method" value="-1" />不同意</td>
				</tr>
				<tr>
					<td class="td_table_1"><span>领导审批意见：</span></td>
					<td class="td_table_2" colspan="3"><textarea
							class="input_textarea_320" id="suggest" name="suggest"></textarea>
					</td>
				</tr>
			</table>
		</fieldset>
		<table align="center" border="0" cellpadding="0" cellspacing="0">
			<tr align="center">
				<td colspan="1" class="h50">
					...
					<input type="button" class="selectBtn" name="reback" value="返回" onclick="bakIframe('${map.label}','${map.bakurl}')">&nbsp;&nbsp;
				</td>
			</tr>
		</table>
	</form>
	...
	<div id="view_rece">
		<div class="outer">
			...
			<div class="content_y">
						<div id="flowTable_rece"></div>
						<div id="formTable_rece"></div>
