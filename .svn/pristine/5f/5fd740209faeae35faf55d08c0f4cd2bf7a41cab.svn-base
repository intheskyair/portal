...
.effeckt-show .effeckt-modal {
	...
	<script type="text/javascript">
	 ...
	 $(function(){
      	...
      	$.ajax({
     		url:"borrow!taskBack.action?orderId=${map.orderId}&processId=${map.processId}",
     		type:"POST",
        	cache: false,
    		success: function(result){
    			var data = eval("("+result+")");
    			for(var i in data){
    				$("#taskBack").append("<option value="+data[i].taskName+">"+data[i].displayName+"</option>");
    			}
    		}   
     	}); 
     	...
     	$('input[name="method"]').click(function(){
			if($(this).val() == "-1"){
				$("#table_bask").css("visibility","visible");
				$("#taskBack").attr("name","taskBack");
			}else{
				$("#taskBack").removeAttr("name","taskBack");
				$("#table_bask").css("visibility","hidden");
			}
     	});
     }); 
	...
	<body style="height:100%;overflow-y: scroll" >
		<form id="inputForm1" action="" method="post" target="mainFrame"  >
           ...
           <legend>【${map.label}】</legend>		
			...
			<tr id="table_bask" style="visibility: hidden;">
				<td class="td_table_1">
					<span>任务驳回：</span>
				</td>
				<td class="td_table_2" colspan="3">
					<select id="taskBack"></select>
				</td>
			</tr>
			</table>
 </fieldset>			
