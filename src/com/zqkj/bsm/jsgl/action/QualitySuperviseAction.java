...
import com.zqkj.bsm.jsgl.dao.QualitySuperviseDao;
...
@Namespace("")
@Results({ 
			@Result(name = "qualitysupervise", location = "/WEB-INF/jsp/jsgl/qualitysupervise.jsp")  
        }) 
@ExceptionMappings({ 
	@ExceptionMapping(exception = "java.lange.RuntimeException", result = "error") 
}) 
@Action(value = "qualitysupervise") 
/**
 * 质量监督
 * @author xj
 *
 */
public class QualitySuperviseAction extends BaseAction{
    ...
    public void exportJobs() throws IOException 
    {
		...
		List<Map<String, Object>> items = QualitySuperviseDao.getInstance().getExportDara(params_all);
		
		String file_name = "质量监督列表";
		//导出的字段和excel表头的对应关系
        Map<String, String> map_columns = QualitySuperviseDao.getInstance().getList_Column();
 ...
 * @return
 */
	public String getQualitySuperviseList()
	{
        ...
        if("init".equals(operation))
        {
            ...
            Map<String, Object> params_all = this.dealParams(paraNames,null);
            //分页封装方法
            OraPaginatedList list_tmp = QualitySuperviseDao.getInstance().queryQualitySupervise(params_all, pageNum, pageSize);
            // 分页对象
	 ...
	 * @throws Exception
	 */
    public void addQualitySupervise() throws Exception
    {
    	...
    	if(QualitySuperviseDao.getInstance().addQualitySupervise(params_all))
    	{
    		res_return= "{msg:\"ok\"}";
    	}
    	else
    	{
    		res_return="{msg:\"保存质量监督失败\"}";
    	}
     ...
     * @throws Exception
     */
    public void updateQualitySupervise() throws Exception
    {
        response.setContentType("text/html; charset=UTF-8");
        Enumeration<String> paraNames_ = request.getParameterNames();
        Map<String, Object> params_all = this.dealParams(paraNames_,null);
        String res_return="";
        boolean flag = QualitySuperviseDao.getInstance().updateQualitySupervise(params_all);
    	if(flag)
    	{
    		res_return= "{msg:\"ok\"}";
    	}
     ...
     * 删除质量监督
     */
    public void deleteQualitySupervise()
    {
        response.setContentType("text/html; charset=UTF-8");
        String id  = StringHelper.noEmpty2(request.getParameter("id")).trim();
        String res_return="";
    	if(QualitySuperviseDao.getInstance().deleteQualitySupervise(id))
    	{
    		res_return= "{msg:\"ok\"}";
    	}
    	else
    	{
    		res_return="{msg:\"删除质量监督失败\"}";
    	}
